(ns atsketch.cocktails.sandjin
  (:require [quil.core :as q]
            ...
            [atsketch.cocktails.primitives :as prim]))
...
(defn draw-state [{:keys [background go parts fonts strawberry glitter signature-alpha] :as s}]
  (when go
    ...
    (prim/draw-description :lines [{:text "Sand Jinn" :color [155 255 255 220] :size 26 :offset -10 :line-height 1.1}
        ...
        (merge
         ...
         {:step (cond
          ...
          :parts [{:y (+ (* 0.5 h) (* 0 (+ layer-h layer-displ)))
                   ...
                   :noise (prim/gen-noise-in-rect 2000 layer-w layer-h)}
                  ...
                  {:y (+ (* 0.5 h) (+ layer-h layer-displ) (+ (* 0.75 layer-h) layer-displ))
                   ...
                   :noise (prim/gen-noise-in-rect 1000 layer-w (* 0.5 layer-h))}]
          :pixels (->> (fn []
